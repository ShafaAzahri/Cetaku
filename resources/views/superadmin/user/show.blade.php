...
@section('content')
  <div class="container-fluid">
    ...
    <div class="card-body">
      ...
      <img src="https://ui-avatars.com/api/?name={{ urlencode($user['nama']) }}&background=4361ee&color=fff&size=100"
        class="admin-avatar-large" alt="{{ $user['nama'] }}">
      <div>
        <h5 class="admin-name">{{ $user['nama'] }}</h5>
      ...
      @endif
        ...
        <div class="admin-info-value">{{ $user['last_login_ip'] ?? '-' }}</div>
        </div>
      </div>
      </div>

      <div class="mt-4 d-flex">
      <a href="{{ route('superadmin.user.edit', $user['id']) }}" class="btn btn-warning action-btn">
  ...
  <div class="modal fade" id="resetPasswordModal" tabindex="-1" aria-labelledby="resetPasswordModalLabel"
    ...
    <div class="modal-content">
      ...
      <div class="modal-body">
      <p>Apakah Anda yakin ingin mereset password untuk <strong>{{ $user['nama'] }}</strong>?</p>
      <p>Password baru akan ditampilkan setelah reset berhasil.</p>
      </div>
      <div class="modal-footer">
      <button type="button" class="btn btn-secondary" data-bs-dismiss="modal">Batal</button>
      <form action="{{ route('superadmin.user.reset-password', $user['id']) }}" method="POST">
        @csrf
  ...
  <div class="modal fade" id="newPasswordModal" tabindex="-1" aria-labelledby="newPasswordModalLabel" aria-hidden="true">
    ...
    <div class="modal-content">
      ...
      <div class="modal-body">
      <p>Password untuk user <strong>{{ $user['nama'] }}</strong> telah direset.</p>
      <div class="alert alert-info">
        <p class="mb-0">Password baru: <strong id="new-password">{{ session('new_password') }}</strong></p>
      </div>
      <p class="text-danger">Pastikan Anda menyimpan password ini di tempat yang aman, karena ini hanya akan
